...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation that closely reflect real-world conditions. You MUST return valid JSON format. All string values must not contain unescaped newline characters."
        return f"{base_prompt}\n\nIMPORTANT: {get_language_instruction()} This applies to all natural language text fields (bio, persona, profession, interested_topics, country). JSON field names and gender/mbti values must remain in English."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the following entity, closely reflecting real-world conditions.
...
2. persona: Detailed persona description (~2000 words, plain text), including:
   ...
   - Background (key experiences, relation to the event, social connections)
   ...
   - Stance/views (attitude toward the topic, what triggers or moves them)
   - Unique traits (catchphrases, special experiences, hobbies)
   - Personal memories (important part of the persona: this individual's connection to the event and their actions/reactions)
...
- {get_language_instruction()} (gender field must remain English: male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for the following institutional/group entity, closely reflecting real-world conditions.
...
2. persona: Detailed account profile description (~2000 words, plain text), including:
   ...
   - Content characteristics (content types, posting frequency, peak activity periods)
   - Stance (official position on core topics, how the institution handles controversy)
   - Special notes (audience profile, operational habits)
   - Institutional memory (important part of the persona: this institution's connection to the event and its actions/reactions)
...
5. mbti: MBTI type describing the account's style (e.g. ISTJ for conservative/structured)
6. country: Country name
...
- persona must be a single coherent paragraph of text, no newline characters
- {get_language_instruction()} (gender field must remain English: "other")
- age must be integer 30, gender must be string "other"
- The account's communication must match its institutional identity"""
                ...
                try:
                    ...
                    if progress_callback:
                        progress_callback(
                            current,
                            total,
                            t('progress.profileCompleted', current=current, total=total, name=entity.name, entity_type=entity_type)
                        )
